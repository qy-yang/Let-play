...
class GameField(object):
            ...
            def merge(row):
                ...
                for i in range(len(row)):
                    if pair:  # merge
                        ...
                        self.score += 2 * row[i]
                        pair = False
                    else:  # check if there are pairs
                        if (i + 1) < len(row) and row[i] == row[i + 1]:
                            pair = True
                            new_row.append(0)  # not change the length of a row, zeros will be tightened 
                        else:
    ...
    def draw(self, screen):
        ...
        def draw_row(row):
            cast(''.join('|{: ^5} '.format(num) if num > 0 else '|      ' for num in row) + '|')  # center alignment
...
def main(stdscr):
    ...
    def not_game(state):
        ...
        # Get user's action, check if end of game
